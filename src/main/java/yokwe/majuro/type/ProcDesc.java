...
public final class ProcDesc {
    ...
    public static ProcDesc value(int value) {
        return new ProcDesc(NO_VALUE, NO_VALUE, value, false);
    }
    public static ProcDesc fetch(int base) {
        int ra0 = Mesa.fetch(base + 0);
        int ra1 = Memory.isSamePage(base,  base + 1) ? ra0 + 1 : Mesa.fetch(base + 1);
        return new ProcDesc(ra0, ra1, Mesa.readReal32(ra0, ra1), false);
    }
    public static ProcDesc store(int base) {
        int ra0 = Mesa.store(base + 0);
        int ra1 = Memory.isSamePage(base,  base + 1) ? ra0 + 1 : Mesa.store(base + 1);
        return new ProcDesc(ra0, ra1, Mesa.readReal32(ra0, ra1), true);
    }
    ...
    private ProcDesc(int ra0, int ra1, int value, boolean canWrite) {
        this.ra0      = ra0;
        this.ra1      = ra1;
